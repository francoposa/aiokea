...
async def test_where(db, user_pg_client):
    ...
    # Filters
    result_include = await user_pg_client.where(
        inclusion_map={"username": ["brian", "roman"]}
    )
    result_exclude = await user_pg_client.where(
...
async def test_where_paginated(db, user_pg_client):
    db_count = len(await user_pg_client.where())
    retrieved_records = 0
    page = 0
    page_size = 1
    while retrieved_records < db_count:
        results = await user_pg_client.where(page=page, page_size=page_size)
